...
public final class ImageScannerController: UINavigationController {
    ...
    public required init(image: UIImage? = nil, delegate: ImageScannerControllerDelegate? = nil,isGallery:Bool = false) {
        ...
        if let image = image {
            detect(image: image) { [weak self] detectedQuad in
                ...
                print(#function)
                let editViewController = EditScanViewController(image: image, quad: detectedQuad, rotateImage: isGallery)
